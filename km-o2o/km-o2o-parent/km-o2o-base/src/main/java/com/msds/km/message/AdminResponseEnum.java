 ...
 * @since JDK 1.6.0_21  
 */
public enum AdminResponseEnum implements DescriptionEnum{
	...
	private AdminResponseEnum(int id,String name){
		this.id = id;
		this.name = name;
	}
	...
	public static AdminResponseEnum getEnumById(int id){
		AdminResponseEnum [] values = AdminResponseEnum.values();
		for(AdminResponseEnum en : values){
			if(en.getId() == id){
				return en;
			}
		}
	...
	public static List<DescriptionEnum> getValues() {
		...
		AdminResponseEnum [] values = AdminResponseEnum.values();
		for(AdminResponseEnum val : values){
			result.add(val);
		}
		return result;
	}
