...
//		}
	...
	public @ResponseBody String  uploadFile(String dirName,HttpServletRequest request, HttpServletResponse response) throws Exception{
		MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
		Map<String, MultipartFile> fileMap = multipartRequest.getFileMap();
		String contextPath=request.getSession().getServletContext().getRealPath("/");
		String sep = System.getProperty("file.separator");
		...
		String date = format02(new Date());
		String baseDir ="upload" + sep + "files";
		if(dirName == null || dirName.equals("")){
			dirName = "test";
		}
		String filePath=baseDir + sep + dirName+sep+date;//给商品配置文件夹
