...
//		 action列，超连接操作按钮
	 ...
	 //vin不为空，可添加保养记录
	 if (field == "action") {
		 if(record.vin != "" && record.vin!= undefined){
			 e.cellStyle = "text-align:center";
		     e.cellHtml = '<a href="javascript:addHistory(\'' + record.vin +'\')">录入保养记录</a>&nbsp';
		 }
	 };
...
function addHistory(vin) {
	parent.addTabUrl(ctx+'/maintenanceHistory/editMaintenance.action?vin='+vin,'新增保养记录');
}
