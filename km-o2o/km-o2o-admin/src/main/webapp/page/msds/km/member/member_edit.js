...
//	 action列，超连接操作按钮
	 if (field == "action") {
	     ...
	     if(record.bespeakType  ==  1){
	    	 e.cellHtml = '<a href="javascript:getBespeakInfo(\'true\',\'' + record.id + '\',\'编辑预约单\')">修改</a>&nbsp; ';
	     }else{
	    	 e.cellHtml = '<a href="javascript:getBespeakOnlineInfo(\'true\',\'' + record.id + '\',\'编辑预约单\')">修改</a>&nbsp; ';
	     }
...
showGroupNameRow();
/**
 * 如果modelId有值则不显示参考车型行
 */
function showGroupNameRow(){
	var id = mini.get("id").getValue();
	var modelGroupName = mini.get("modelGroupName").getValue();
	//modelId有值、不是新增、modelGroupName有值则显示
	if((year.getValue()==null || year.getValue()=="" || year.getValue()==0)
			&& (id!=null && id!="")
			&& (modelGroupName!=null && modelGroupName!="")){
		document.getElementById("groupNameRow").style.display = "";
	}
}
